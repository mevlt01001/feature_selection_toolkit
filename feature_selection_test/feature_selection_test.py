...
class TestFeatureSelectionMethods(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.X_classification, self.y_classification = make_classification(n_samples=100, n_features=8, random_state=42)
        self.X_classification = pd.DataFrame(self.X_classification, columns=[f'Feature_{i}' for i in range(8)])
        self.y_classification = pd.Series(self.y_classification, name='Target')

        # Regression dataset
        self.X_regression, self.y_regression = make_regression(n_samples=100, n_features=8, random_state=42)
        self.X_regression = pd.DataFrame(self.X_regression, columns=[f'Feature_{i}' for i in range(8)])
    ...
    def test_filter_method(self):
        ...
        assert len(scores) == self.X_classification.shape[1]
        assert len(p_values) == self.X_classification.shape[1]
